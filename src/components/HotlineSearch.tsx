...
export default function HotlineSearch() {
  ...
  async function searchLocal(categoryId: string) {
    ...
    try {
      const clauses = category.osm.map(selector => `nwr${selector}(around:20000,${location.lat},${location.lng});`).join('\n');
      const res = await fetch('https://overpass-api.de/api/interpreter', { method: 'POST', body: `[out:json][timeout:25];(${clauses});out center tags;` });
      if (!res.ok) throw new Error(`Public directory returned HTTP ${res.status}`);
      const data = await res.json();
      const mapped: LocalResult[] = (data.elements || []).map((el: any) => {
